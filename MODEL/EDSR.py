...
from UTILS.util import del_file,setup_logger,fspecial,DegradeFilter
import logging
...
class EDSR(BasicModel):
    ...
    def edsr(self,x):
        ...
        y = y+self.bias_three
        if self.scale == 1: return tf.identity(y,name = 'EDSR')
        y = tf.nn.depth_to_space(y,self.scale,data_format = 'NHWC', name = 'NHWC_output')
        
        return tf.identity(y,name = 'EDSR')
    ...
    def train_scale(self, imagefolder,validfolder):
        ...
        if os.path.exists('./LOGS/EDSR'):
            del_file("./LOGS/EDSR/")
        summary_writer = tf.summary.FileWriter('./LOGS/EDSR', sess.graph)
        ...
        for ep in range(epoch_start, self.epochs):
            ...
            for idx in range(batch_start, batch_num):
                ...
                _, err, lr = sess.run([learning_step, loss, learning_rate], feed_dict={images: batch_images, labels: batch_labels})
                ...
                if counter % 500 == 0:
                    ...
                    for p in test_path:
                        ...
                        sr = sess.run(pred, feed_dict={images: input_})
                        sr = np.squeeze(sr)
                        sr = np.clip(sr, 0, 255)
                    ...
                    if avg_psnr>flag:
                        ...
                        constant_graph = graph_util.convert_variables_to_constants(sess, sess.graph_def, ["EDSR"])
                        with tf.gfile.FastGFile(self.save_path+'EDSR_x{}.pb'.format(self.scale), mode='wb') as f:
                            f.write(constant_graph.SerializeToString())
                        flag = avg_psnr
                    summary_str = sess.run(merged_summary_op, feed_dict={images: batch_images, labels: batch_labels})
                ...
                if counter > 0 and counter == batch_num * self.epochs:
                    ...
                    for p in test_path:
                        input_, label_ = super().get_image(p, self.scale, None)
                        sr = sess.run([pred], feed_dict={images: input_ / 255.0})
